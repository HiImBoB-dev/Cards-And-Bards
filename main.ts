...
function StartGame () {
    CD = cardKit.createPlayingCards()
    cardKit.shuffleCards(CD)
    for (let RV of cardKit.getLayoutCardListCopy(CD)) {
        if (cardKit.getCardNumberAttribute(RV, CardAttributes.Rank) >= 11) {
            cardKit.setCardNumberAttribute(RV, CardAttributes.Rank, 10)
        }
    }
    cardKit.setContainerPosition(CD, 10, 110)
    HD = cardKit.createEmptyHand(DesignTemplates.PlayingCards, CardContainerKinds.Draw, CardLayoutDirections.CenteredLeftRight)
    cardKit.setContainerPosition(HD, 80, 105)
    for (let index = 0; index < 7; index++) {
        cardKit.moveCardBetween(CD, CardContainerPositions.First, HD, CardContainerPositions.First, CardFaces.Up)
    }
    cardKit.moveCursorInsideLayoutWithButtons(HD)
    cardKit.setCursorAnchor(AnchorPositions.Center, 0, 0)
    scene.setBackgroundImage(assets.image`myImage`)
}
let HD: cardCore.CardContainer = null
let CD: cardCore.CardContainer = null
let GS = false
let TS: fancyText.TextSprite = null
if (false) {
    scene.setBackgroundImage(img`
        ...
        `)
    TS = fancyText.create("Cards And Bards  ", 0, 9, fancyText.serif_small)
...
game.onUpdateInterval(200, function () {
    for (let RV of cardKit.getLayoutCardListCopy(HD)) {
        if (RV == cardKit.getCursorCard()) {
            RV.y = 100
            timer.after(500, function () {
                sprites.destroy(TS)
                TS = fancyText.create("" + cardKit.getCardTextAttribute(RV, CardAttributes.Rank) + " of " + cardKit.getCardTextAttribute(RV, CardAttributes.Suit), 70, 4, fancyText.geometric_serif_6)
                fancyText.setFrame(TS, img`
